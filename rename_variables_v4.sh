...
# Path to injective-helix repository
...
find "$REPO_PATH" -type f \( -name "*.ts" -o -name "*.js" -o -name "*.vue" \) \
  ...
  # Also handle variables that already have "In" prefix but need to be changed to "in"
  ...
  # Handle variables that have "In" in the middle (like quantityInChainFormat)
  sed -i '' -E 's/\b([a-zA-Z_][a-zA-Z0-9_]*)In(ChainFormat|HumanReadable|BigNumber)\b/\1in\2/g' "$file"
done
